...
### 注意：更新链码就是重新部署一次链码，无需初始化，序列号加一，版本加一
### 前提条件
虚拟机已完成以下环境配置：
...
```
#### 6.3 测试CreateBOM
**注意：CreateBOM需要Org1（零部件生产厂商）身份，需先切换到Org1环境变量**
```bash
# 切换到Org1环境变量
export PATH=~/fabric/fabric-samples/bin:$PATH
export FABRIC_CFG_PATH=~/fabric/fabric-samples/config
...
# 执行CreateBOM命令
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls true --cafile ${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C test -n test1 --peerAddresses localhost:7051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt --peerAddresses localhost:9051 --tlsRootCertFiles ${PWD}/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt -c '{"function":"CreateBOM","Args":["{\"bomID\":\"R-BOM-202505-002\",\"bomType\":\"研发BOM\",\"productModel\":\"2025款XX车型\",\"partBatchNo\":\"B20250502\",\"version\":\"V1.0\",\"creator\":\"厂商A\",\"createTime\":\"1735689600\",\"status\":\"已发布\",\"materialList\":[{\"materialID\":\"MAT-001\",\"materialName\":\"发动机活塞\",\"spec\":\"10±0.1mm\",\"quantity\":100,\"supplierID\":\"SUP-001\"},{\"materialID\":\"MAT-002\",\"materialName\":\"活塞环\",\"spec\":\"5±0.05mm\",\"quantity\":200,\"supplierID\":\"SUP-002\"}],\"rdBOMFileInfo\":null,\"productionBOMInfo\":null,\"changeRecords\":[]}"]}'
```
#### 6.3.1 查询BOM(测试QueryBOM)
```bash
peer chaincode query -C test -n test1 -c '{"Args":["QueryBOM","R-BOM-202505-002"]}'
# 预期输出：包含完整BOM信息的JSON对象
```
