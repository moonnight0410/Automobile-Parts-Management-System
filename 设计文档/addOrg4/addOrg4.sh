...
# 此脚本通过添加第四个组织来扩展 Hyperledger Fabric 测试网络
...
function printHelp () {
  ...
  echo "  addOrg4.sh -h|--help (打印此消息)"
  echo "    <mode> - 以下之一: 'up', 'down', 或 'generate'"
  echo "      - 'up' - 将 org4 添加到示例网络。您需要先启动测试网络并创建通道。"
  echo "      - 'down' - 关闭测试网络和 org4 节点"
...
# 确定是启动、停止、重新启动还是生成公告
if [ "$MODE" == "up" ]; then
  infoln "将 org4 添加到通道 '${CHANNEL_NAME}'，超时时间为 '${CLI_TIMEOUT}' 秒，CLI 延迟为 '${CLI_DELAY}' 秒，使用数据库 '${DATABASE}'"
  echo
elif [ "$MODE" == "down" ]; then
  EXPMODE="停止网络"
elif [ "$MODE" == "generate" ]; then
  EXPMODE="为 Org4 生成证书和组织定义"
else
  printHelp
  exit 1
fi
